...
namespace MrrHak.Extensions.FileProviders.S3FileProvider
{
    ...
    public static class S3FileProviderExtensions
    {
        ...
        public static S3FileProvider GetS3FileProvider(this IServiceProvider serviceProvider, string bucketName, string? rootPath = null)
        {
            ...
            return new S3FileProvider(amazonS3, bucketName, rootPath);
        }
        ...
        public static IApplicationBuilder UseS3StaticFiles(this IApplicationBuilder app, string bucketName, string? requestPath = null, string? rootPath = null, bool? serveUnknownFileTypes = false)
        {
            var s3FileProvider = app.ApplicationServices.GetS3FileProvider(bucketName, rootPath);
        ...
        {
            var s3FileProvider = app.ApplicationServices.GetS3FileProvider(options.BucketName, options.RootPath);
            var staticFilesOption = new StaticFileOptions() { FileProvider = s3FileProvider };
        ...
        /// <param name="serveUnknownFileTypes">If the file is not a recognized content-type should it be served? Default: false</param>
        /// <returns>The updated application builder.</returns>
        public static IAppBuilder UseS3StaticFiles(this IAppBuilder app, IAmazonS3 amazonS3, string bucketName, string? requestPath = null, string? rootPath = null, bool? serveUnknownFileTypes = false)
        {
            Microsoft.Owin.StaticFiles.StaticFileOptions staticFileOptions = new() { FileSystem = new S3OwinFileSystem(amazonS3, bucketName, rootPath) };
